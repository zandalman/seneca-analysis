        ...
        if os.path.getsize(plot_data_file_path) > 0:  # ignore empty files
            ...
            for plot_data in plot_data_list:
                if plot_data["type"] == "message":
                    ...
                    if CurrentPlot.query.filter(CurrentPlot.plot_id==obj.id).count() == 0:
                        if CurrentPlot.query.filter(CurrentPlot.file==obj.file).count() == 0: # check if data is from a new routine
                        ...
                        db.session.add(CurrentPlot(obj.file, obj.id))
                        db.session.commit()
                    else:
