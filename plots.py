...
def create_plot(obj_response, plot):
    plot_id = "%s-%s-%s" % (plot["file"], plot["name"], plot["counter"])
    ...
    obj_response.html_append("#plot-list-%s" % plot["file"], "<li class='plot-list-item invisible' data-id='%s'>%s</li>" % (plot_id, plot["name"]))
    ...
    if plot["data"]:
        ...
        obj_response.html_append("#plot-list-%s" % plot["file"], "<li class='plot-list-item invisible' data-id='%s-table'>%s - table</li>" % (plot_id, plot["name"]))
...
def create_routine(obj_response, routine_name):
    ...
    obj_response.html_append("#plot-list", "<ul class='plot-list-routine' id='plot-list-%s'>%s</ul>" % (routine_name, routine_title_HTML))
    yield obj_response
